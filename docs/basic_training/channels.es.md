---
description: Taller de Entrenamiento Básico de Nextflow
---

# Canales

Los canales son una estructura de datos clave en Nextflow que permiten la implementación de flujos de trabajo computacionales orientados a funciones reactivas basados en el paradigma de programación [Dataflow](https://en.wikipedia.org/wiki/Dataflow_programming).

Se utilizan para conectar tareas entre sí de forma lógica o para implementar transformaciones de datos de estilo funcional.
...
Nextflow distingue dos tipos diferentes de canales: canales de **cola** y canales de **valor**.
  
### Canal de cola

Un canal de _cola_ es una cola _FIFO_ (First-in-First-out - primero en entrar, primero en salir) _asincrónica_ unidireccional que conecta dos procesos u operadores.

- _asincrónico_ significa que las operaciones ocurren sin bloqueo.
- _unidireccional_ significa que los datos fluyen de un generador a un consumidor.
- _FIFO_ significa que se garantiza que los datos se entregarán en el mismo orden en que se producen. Primero en entrar primero en salir.
  
Un canal de cola se crea implícitamente mediante definiciones de salida de un proceso o mediante fábricas de canales como [Channel.of](https://www.nextflow.io/docs/latest/channel.html#of) o [Channel.fromPath](https://www.nextflow.io/docs/latest/channel.html#frompath).
  
Prueba los siguientes fragmentos de código:
  
!!! info ""

    Haz click en los iconos :material-plus-circle: en el código para explicaciones.
...
1. Usa la función de impresión integrada `println` para imprimir el canal `ch`
2. Aplicar el método `view` al canal `ch` imprime cada elemento emitido por los canales

!!! exercise

    Intenta ejecutar este fragmento. Puedes hacerlo creando un nuevo archivo `.nf` o editando un archivo `.nf` ya existente.
...
Un canal de **valor** (también conocido como canal singleton), por definición, está vinculado a un solo valor y se puede leer un número ilimitado de veces sin consumir su contenido. Un canal de `valor` se crea utilizando el método de fábrica de canal de valor [value](https://www.nextflow.io/docs/latest/channel.html#value) o mediante operadores que devuelven un solo valor, como [first](https://www.nextflow.io/docs/latest/operator.html#first), [last](https://www.nextflow.io/docs/latest/operator.html#last), [collect](https://www.nextflow.io/docs/latest/operator.html#operator-collect), [count](https://www.nextflow.io/docs/latest/operator.html#operator-count), [min](https://www.nextflow.io/docs/latest/operator.html#operator-min), [max](https://www.nextflow.io/docs/latest/operator.html#operator-max), [reduce](https://www.nextflow.io/docs/latest/operator.html#operator-reduce) y [sum](https://www.nextflow.io/docs/latest/operator.html#operador-sum).
Para comprender mejor la diferencia entre los canales de valor y de cola, guarda el fragmento a continuación como `example.nf`.
...
Cuando ejecutas el script, verás que imprime solo 2, como se puede ver a continuación:
  
```console
2
```

Para entender por qué, podemos inspeccionar el canal de cola y ejecutar Nextflow con DSL1 nos brinda una comprensión más explícita de lo que está sucediendo tras bambalinas.
...
```
Tenemos el valor 1 como único elemento de nuestro canal de cola y una píldora venenosa, que le dirá al proceso que no queda nada por consumir. Es por eso que solo tenemos una salida para el ejemplo anterior, que es 2. Inspeccionemos ahora un canal de valor.
...
```
No existe una píldora venenosa, y es por eso que obtenemos una salida diferente con el siguiente código, donde `ch2` se convierte en un canal de valor a través del operador `first`.
...
Además, en muchas situaciones, Nextflow convertirá implícitamente las variables en canales de valor cuando se utilicen en la invocación de un proceso. Por ejemplo, cuando invoca un proceso con un parámetro de pipeline (`params.example`) que tiene un valor de string, se convierte automáticamente en un canal de valor.

## Fábricas de canal 

Estos son comandos de Nextflow para crear canales que poseen entradas y funciones implícitas esperadas.

### `value()`
  
El método de fábrica de canal `value` se usa para crear un canal _value_. Se puede especificar un argumento opcional no "nulo" para vincular el canal a un valor específico. Por ejemplo:
...
1. Crea un canal de valor _vacío_
2. Crea un canal de valor y le vincula un string
3. Crea un canal de valor y le vincula un objeto de lista que se emitirá como una emisión única.

### `of()`

La fábrica de canal `Channel.of` permite la creación de un canal de cola con los valores especificados como argumentos.
...
La primera línea de este ejemplo crea una variable `ch` que contiene un objeto de canal. Este canal emite los valores especificados como parámetro en el método `of`. Así la segunda línea imprimirá lo siguiente:
